...
/// CP-Token (Confidential Performant Token) — P-Token rebuilt with
...
/// PDA: `["cp_receipt", burned_ct]`. Closed after unwrap completes.
#[repr(C)]
...
fn initialize_mint(program_id: &Address, accounts: &[AccountView], data: &[u8]) -> ProgramResult {
    ...
    let bb = [bump];
    let seeds = [Seed::from(b"cp_mint" as &[u8]), Seed::from(authority.address().as_ref()), Seed::from(&bb)];
...
fn initialize_account(program_id: &Address, accounts: &[AccountView], data: &[u8]) -> ProgramResult {
    ...
    let bb = [ab];
    let seeds = [Seed::from(b"cp_account" as &[u8]), Seed::from(mint_acct.address().as_ref()), Seed::from(owner.address().as_ref()), Seed::from(&bb)];
...
fn initialize_vault(program_id: &Address, accounts: &[AccountView], data: &[u8]) -> ProgramResult {
    let [vault_acct, cp_mint, spl_mint, payer, _sys, ..] = accounts else { return Err(ProgramError::NotEnoughAccountKeys); };
    if !payer.is_signer() { return Err(ProgramError::MissingRequiredSignature); }
    if data.is_empty() { return Err(ProgramError::InvalidInstructionData); }
    let vb = data[0]; let md = unsafe { cp_mint.borrow_unchecked() };
    if !Mint::from_bytes(md)?.is_initialized() { return Err(ProgramError::UninitializedAccount); }
    let bb = [vb];
    let seeds = [Seed::from(b"cp_vault" as &[u8]), Seed::from(cp_mint.address().as_ref()), Seed::from(&bb)];
...
// ── 31: UnwrapBurn ──
// Burns cpTokens and creates a temporary WithdrawalReceipt.
...
fn unwrap_burn(program_id: &Address, accounts: &[AccountView], data: &[u8]) -> ProgramResult {
    ...
    // Create receipt PDA
    let bb = [rb];
    let seeds = [Seed::from(b"cp_receipt" as &[u8]), Seed::from(burned_ct.address().as_ref()), Seed::from(&bb)];
...
fn unwrap_complete(accounts: &[AccountView]) -> ProgramResult {
    let [receipt_acct, vault_acct, cp_mint, req_acct, vault_ata, user_ata, owner, destination, _spl, ..] = accounts
    else { return Err(ProgramError::NotEnoughAccountKeys); };
    if !owner.is_signer() { return Err(ProgramError::MissingRequiredSignature); }
    let rd = unsafe { receipt_acct.borrow_unchecked() };
    let r = WithdrawalReceipt::from_bytes(rd)?;
    ...
    if &vad[32..64] != vault_acct.address().as_ref() { return Err(ProgramError::InvalidArgument); }
    let vbb = [vault.bump];
    let vs = [Seed::from(b"cp_vault" as &[u8]), Seed::from(cp_mint.address().as_ref()), Seed::from(&vbb)];
