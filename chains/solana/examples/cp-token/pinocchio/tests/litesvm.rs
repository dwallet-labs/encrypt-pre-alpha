...
//! LiteSVM end-to-end tests for CP-Token (Confidential Performant Token).
//!
...
fn create_mint(
    ...
    let (mint_pda, bump) = Pubkey::find_program_address(
        &[b"cp_mint", authority.pubkey().as_ref()],
        program_id,
    );
...
fn create_account(
    ...
    let (account_pda, account_bump) = Pubkey::find_program_address(
        &[b"cp_account", mint.as_ref(), owner.pubkey().as_ref()],
        program_id,
    );
