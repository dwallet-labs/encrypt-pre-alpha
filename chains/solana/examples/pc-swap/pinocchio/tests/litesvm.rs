...
//! LiteSVM tests for PC-Swap with enforced LP positions.
...
const PROG: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../../../../../target/deploy/pc_swap.so");
...
fn mk_pool(ctx: &mut EncryptTestContext, pid: &Pubkey, auth: &Pubkey, bump: u8) -> PoolInfo {
    ...
    let (pp, pb) = Pubkey::find_program_address(&[b"pc_pool", ma.as_ref(), mb.as_ref()], pid);
...
fn mk_lp(ctx: &mut EncryptTestContext, pid: &Pubkey, auth: &Pubkey, bump: u8,
    ...
    let (lp_pda, lp_bump) = Pubkey::find_program_address(
        &[b"pc_lp", pool.pda.as_ref(), owner.as_ref()], pid);
...
fn add_liq(ctx: &mut EncryptTestContext, pid: &Pubkey, auth: &Pubkey, bump: u8,
    ...
    let mut keys = vec![
        ...
        AccountMeta::new_readonly(Pubkey::find_program_address(&[b"pc_lp", pool.pda.as_ref(), &[0;32]], pid).0, false), // dummy, we use user_lp_ct directly
    ...
    let (lp_pda, _) = Pubkey::find_program_address(&[b"pc_lp", pool.pda.as_ref(), owner.as_ref()], pid);
    let mut keys2 = vec![
        AccountMeta::new_readonly(pool.pda, false),
...
#[test] fn test_cannot_use_other_users_lp() {
    ...
    let (lp_pda, _) = Pubkey::find_program_address(
        &[b"pc_lp", pool.pda.as_ref(), other_owner.as_ref()], &pid);
    let mut keys = vec![
        AccountMeta::new_readonly(pool.pda, false),
        AccountMeta::new_readonly(lp_pda, false),
        AccountMeta::new(pool.ra, false), AccountMeta::new(pool.rb, false),
