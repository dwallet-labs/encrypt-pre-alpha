#!/usr/bin/env bun
/**
 * CP-Token E2E — USDC → cpUSDC → USDC (fully confidential)
 *
...
import { type CpTokenContext, deriveCpTokenPdas, deriveMintPda, deriveAccountPda, deriveVaultPda, deriveReceiptPda, initializeMintIx, initializeAccountIx, initializeVaultIx, transferIx, wrapIx, unwrapBurnIx, unwrapDecryptIx, unwrapCompleteIx } from "./instructions.ts";
...
if (!eArg || !tArg) { console.error("Usage: bun main.ts <ENCRYPT_ID> <CP_TOKEN_ID>"); process.exit(1); }
...
async function doUnwrap(
  grpc: any, ctx: CpTokenContext, cpMint: PublicKey, vault: PublicKey,
  ...
  log(label, `Unwrap burn ${Number(amount)/1e6} cpUSDC...`);
  ...
  await sendTx(conn, payer, [
    unwrapCompleteIx(TP, receiptPda, vault, cpMint,
...
async function main() {
  ...
  const { cpiAuthority, cpiBump } = deriveCpTokenPdas(TP, payer.publicKey);
  const ctx: CpTokenContext = { programId:TP, enc:encA, payer:payer.publicKey, cpiAuthority, cpiBump };
  ...
  log("2/9", "Creating cpUSDC...");
  const mintAuth = Keypair.generate();
  const [cpMint, cpMintBump] = deriveMintPda(TP, mintAuth.publicKey);
  await sendTx(conn, payer, [initializeMintIx(ctx, cpMint, cpMintBump, DECIMALS, mintAuth.publicKey)], [mintAuth]);
  const [vaultPda, vaultBump] = deriveVaultPda(TP, cpMint);
  await sendTx(conn, payer, [initializeVaultIx(ctx, vaultPda, vaultBump, cpMint, usdcMint.publicKey)]);
  const vaultAta = await createSplTokenAccount(conn, payer, usdcMint.publicKey, vaultPda);
  const [aliceCp, aliceBump] = deriveAccountPda(TP, cpMint, alice.publicKey);
  const aliceBal = Keypair.generate();
  await sendTx(conn, payer, [initializeAccountIx(ctx, aliceCp, aliceBump, cpMint, alice.publicKey, aliceBal.publicKey)], [aliceBal]);
  const [bobCp, bobBump] = deriveAccountPda(TP, cpMint, bob.publicKey);
  const bobBal = Keypair.generate();
  await sendTx(conn, payer, [initializeAccountIx(ctx, bobCp, bobBump, cpMint, bob.publicKey, bobBal.publicKey)], [bobBal]);
  const [markCp, markBump] = deriveAccountPda(TP, cpMint, mark.publicKey);
  const markBal = Keypair.generate();
  await sendTx(conn, payer, [initializeAccountIx(ctx, markCp, markBump, cpMint, mark.publicKey, markBal.publicKey)], [markBal]);
  ok("Setup complete");
  ...
  await doUnwrap(grpc, ctx, cpMint, vaultPda, vaultAta.publicKey, bobCp, bobBal.publicKey, bobAta.publicKey, bob, USDC(5), nk, "5/9");
  val("Bob USDC", `${Number(await readSplBalance(conn, bobAta.publicKey))/1e6}`);

  log("6/9", "Alice → Mark: 3 cpUSDC...");
  ...
  await doUnwrap(grpc, ctx, cpMint, vaultPda, vaultAta.publicKey, markCp, markBal.publicKey, markAta.publicKey, mark, USDC(2), nk, "7/9");
  val("Mark USDC", `${Number(await readSplBalance(conn, markAta.publicKey))/1e6}`);

  await doUnwrap(grpc, ctx, cpMint, vaultPda, vaultAta.publicKey, aliceCp, aliceBal.publicKey, aliceAta.publicKey, alice, USDC(1), nk, "8/9");
  ...
  val("  Alice", `${Number(aU)/1e6} USDC + 1 cpUSDC (balance hidden)`);
  val("  Bob  ", `${Number(bU)/1e6} USDC`);
  val("  Mark ", `${Number(mU)/1e6} USDC + 1 cpUSDC (balance hidden)`);
